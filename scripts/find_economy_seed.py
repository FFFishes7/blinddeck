"""One-off: find RED/WHITE seed with Economy Tag on small blind (ante 1).

Record the result in tests/lua/tag_seeds.py as ECONOMY_SMALL.

    python scripts/find_economy_seed.py
"""
...
async def main() -> None:
    ...
    try:
        with httpx.Client(base_url=url, timeout=60.0) as client:
            for i in range(MAX):
                seed = f"S{i:05d}"
                if small_tag_for_seed(client, seed) == ECONOMY_TAG:
                    print(f"FOUND seed={seed!r} small={ECONOMY_TAG!r}")
                    return
                if i and i % 1000 == 0:
                    print(f"checked {i}")
            print("NOT FOUND")
