...
| `save` / `load` / `screenshot` | `PATH`                                 | For `save`/`load`, relative paths are resolved by Balatro/LÖVE, so `save run.jkr` writes to `C:\Users\<username>\AppData\Roaming\Balatro\run.jkr` on Windows. Use an absolute path to save inside this repo.                                                                            |
