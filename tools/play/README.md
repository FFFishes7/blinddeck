...
- **SMODS_BOOSTER_OPENED:** first line under `pack:` is **`choices remaining: N`** — picks still required from **this** open booster (from `G.GAME.pack_choices`). **Charm Tag** skip opens **Mega Arcana** (**2**, then **1** after the first pick); shop normal Arcana is **1**; mega packs from shop are **2** → **1**. Pack rows show target hints such as **`(needs 1-2 targets)`**
    from API `target_min`/`target_max` (hand cards). **`pack IDX [hand indices…]`** — targets are **0-based hand positions** from the `hand:` line, not rank values. Death: **`pack 0 I J`** — card at min(I,J) becomes a copy of card at max(I,J); order of I J doesn't matter. Random-joker Spectrals (Ankh, Hex,
...
| `pack`                         | `IDX [TARGET_IDX...]` or `skip`        | e.g. `pack 0`, `pack 0 1 2` — **hand [N] indices** from `hand:` line (not ranks); Death: `pack 0 I J` (min→copy of max; order irrelevant); Ankh/Hex/Ectoplasm ignore targets (`random_joker_effect`); **`pack skip`** forfeits all remaining picks (see `choices remaining:` in glance) |
| `use`                          | `CONSUMABLE_IDX [CARD_IDX...]`         | e.g. `use 0`, `use 0 1 2` — Death (Tarot): `use 0 I J` (min→copy of max; order irrelevant)                                                                                                                                                                                              |
| `menu`                         | —                                      | return to main menu                                                                                                                                                                                                                                                                     |
| `save` / `load` / `screenshot` | `PATH`                                 | Normal `save PATH` prints `save success: PATH` instead of a state summary. For `save`/`load`, relative paths are resolved by Balatro/LÖVE, so `save run.jkr` writes to `C:\Users\<username>\AppData\Roaming\Balatro\run.jkr` on Windows. Use an absolute path to save inside this repo. |
